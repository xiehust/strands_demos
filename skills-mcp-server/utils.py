...
def get_or_create_user_pool_domain(cognito, user_pool_id, domain_name=None):
    """
    Gets an existing domain for a Cognito User Pool or creates a new one if it doesn't exist.
    
    Args:
        cognito: boto3 cognito-idp client
        user_pool_id: The ID of the user pool to get or create a domain for
        domain_name: Optional custom domain name. If not provided, uses a generated name based on user pool ID
        
    Returns:
        dict: Contains domain information including domain name and URL
        
    Raises:
        ClientError: If domain operations fail
    """
    try:
        boto_session = Session()
        region = boto_session.region_name or REGION
        
        # First, check if a domain already exists for this user pool
        try:
            ...
            if existing_domain:
                ...
                print(f"Found existing domain '{existing_domain}' for user pool {user_pool_id}")
            ...
            if e.response['Error']['Code'] == 'ResourceNotFoundException':
                print(f"Error: User pool {user_pool_id} not found")
                return {
                    'user_pool_id': user_pool_id,
                    'error': 'User pool not found',
                    'status': 'ERROR'
                }
            # If any other error occurs during describe, continue to creation attempt
            pass
        
        # Generate domain name if not provided and no existing domain found
        if not domain_name:
            # Remove underscores and convert to lowercase for domain compatibility
            domain_name = user_pool_id.replace("_", "").lower()
        
        # Create the user pool domain
        print(f"Creating new domain '{domain_name}' for user pool {user_pool_id}")
        ...
        print(f"Successfully created domain '{domain_name}' for user pool {user_pool_id}")
        print(f"Domain URL: {domain_url}")
        ...
        return {
            ...
            'user_pool_id': user_pool_id,
            'region': region,
            'cloudformation_domain_name': response.get('CloudFrontDomain', ''),
            'status': 'CREATED'
        }
        ...
        if error_code == 'InvalidParameterException':
            if 'Domain already associated with another user pool' in error_message:
                print(f"Error: Domain '{domain_name}' is already associated with another user pool")
                return {
                    'domain_name': domain_name,
                    'error': 'Domain already exists with different user pool',
                    'status': 'ERROR'
                }
            else:
            ...
            print(f"Error creating domain: {error_code} - {error_message}")
            return {
                'domain_name': domain_name,
        ...
        print(f"Unexpected error with domain operation: {str(e)}")
        return {
            'domain_name': domain_name,
            'error': str(e),
            'status': 'ERROR'
